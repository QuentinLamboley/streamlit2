...
st.set_page_config(page_title="Disponibilités pour entretiens RESOLVE", layout="wide")
...
# Affichage et suppression des disponibilités individuelles
if nom:
    st.subheader(f"📋 Vos disponibilités ({nom})")
    df = charger_disponibilites()
    df_user = df[df["Nom"] == nom]
    if not df_user.empty:
        selected_to_delete = st.multiselect("Sélectionnez les disponibilités à supprimer :", df_user.apply(lambda x: f"{x['Date']} - {x['Heure']}", axis=1))
        if st.button("❌ Supprimer les disponibilités sélectionnées") and selected_to_delete:
            df = df[~df.apply(lambda x: f"{x['Date']} - {x['Heure']}" in selected_to_delete, axis=1)]
            sauvegarder_disponibilites(df)
            st.success("Les disponibilités sélectionnées ont été supprimées !")
            st.experimental_rerun()
        if st.button("❌ Supprimer toutes mes disponibilités"):
            df = df[df["Nom"] != nom]  # Supprimer les entrées de l'utilisateur
            sauvegarder_disponibilites(df)
            st.success("Toutes vos disponibilités ont été supprimées !")
            st.experimental_rerun()
    else:

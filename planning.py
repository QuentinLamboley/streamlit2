...
if st.button("✅ Enregistrer mes disponibilités"):
    if nom and selected_dates and selected_hours:
        ...
        for date in selected_dates:
            for heure in selected_hours:
                df = df.append({"Nom": nom, "Date": date, "Heure": heure}, ignore_index=True)
...
# Affichage des disponibilités enregistrées
st.subheader("📊 Disponibilités enregistrées")
df = charger_disponibilites()
st.dataframe(df)
